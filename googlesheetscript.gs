...
function menuApproveAndPublish() {
  ...
  if (!approved.length) { 
    SpreadsheetApp.getUi().alert('Nothing approved.'); 
    return; 
  }
  ...
  let message = `✅ Success!\n\n`;
  message += `📊 Published: ${result.published_count} transactions\n`;
  message += `🧠 Rules Learned: ${result.rules_created} new rules\n\n`;
  message += `Message: ${result.message}`;
  
  SpreadsheetApp.getUi().alert('Publish Complete', message, SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function menuRuleStats() {
  ...
  try {
    ...
    if (stats.ok) {
      ...
      message += `Manual Rules: ${stats.manual_rules}\n`;
      message += `Auto-Learned Rules: ${stats.auto_learned_rules}\n\n`;
...
function menuRefreshRules() {
  ...
  try {
    const response = UrlFetchApp.fetch(REFRESH_RULES_URL, {
      method: 'POST',
      headers: { 'X-API-Key': API_KEY },
      muteHttpExceptions: true
    });
    ...
    if (response.getResponseCode() !== 200) {
      ui.alert('Refresh Failed', `HTTP ${response.getResponseCode()}: ${response.getContentText()}`, ui.ButtonSet.OK);
      return;
    }
    ...
    if (result.ok) {
      ui.alert('Rules Refreshed', `✅ ${result.message}\n\nTotal Rules: ${result.total_rules}`, ui.ButtonSet.OK);
    } else {
      ui.alert('Refresh Failed', `Error: ${result.message}`, ui.ButtonSet.OK);
    }
  } catch (error) {
    ui.alert('Refresh Error', `Unexpected error: ${error.toString()}`, ui.ButtonSet.OK);
  }
}
